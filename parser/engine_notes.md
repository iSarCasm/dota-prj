...
4. **Combat logs arrive before entity updates on the same tick.** Manta sorts inner demo-packet messages by priority (lower runs first) in `onCDemoPacket` (`manta-master/demo_packet.go`):
   ...
   Trace output can look backwards because entity lines often carry a **later game timestamp** than the combat line they pair with; that is wall-clock on the event, not callback order.
