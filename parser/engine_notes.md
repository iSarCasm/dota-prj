...
4. **Manta demo-packet message order (same tick)** — `github.com/dotabuff/manta` sorts inner messages in `onCDemoPacket` by priority (lower runs first). Within one tick:
   ...
   **Lasthits implication:** queue hero DAMAGE on combat log; bind on the **following entity updates** (forward correlation). Do **not** finalize or consume pending hero-damage from the combat-log callback — busy ticks have many combat lines before any entities run. Calling `closeAllPendingHeroDamage()` on every combat-log line closes pending with 0 candidates and marks it consumed before entities get a chance. Safe closes: entity update (after epsilon) and creep death. Never finalize a batch with 0 candidates (reopen — entities for that tick have not run yet).
