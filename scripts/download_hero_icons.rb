...
CATEGORY_URL = "#{BASE_URL}/Category:Dota_2_hero_icons"
OUTPUT_DIR = 'hero_icons'
...
def extract_file_links(html_content)
  ...
  # Pattern: /commons/File:Name_icon_dota2_gameasset.png
  ...
  html_content.scan(/href=["']([^"']*File:[^"']*\.png[^"']*)["']/i) do |match|
    href = match[0]
    if href.include?('File:') && href.include?('icon') && href.include?('dota2')
...
def main
  puts "Starting Dota 2 hero icon downloader..."
  ...
  puts "Found #{file_names.length} icon files"
  puts
