...
CATEGORY_URL = "#{BASE_URL}/Category:Dota_2_recipe_item_icons"
OUTPUT_DIR = 'recipe_item_icons'
    ...
    begin
      ...
      members.each do |member|
        ...
        if title.start_with?('File:')
          ...
          # Filter for recipe itemicon files (note: recipe icons use "wikiasset" instead of "gameasset")
...
def extract_file_links(html_content)
  ...
  # Pattern: /commons/File:Name_Recipe_itemicon_dota2_wikiasset.png
  html_content.scan(%r{/commons/File:([^"'\s<>]+\.png)}i) do |match|
    file_name = match[0]
    # Filter for recipe itemicon files (recipe icons use "wikiasset" instead of "gameasset")
...
def main
  puts "Starting Dota 2 recipe item icon downloader..."
  ...
  file_names = get_files_via_api('Category:Dota_2_recipe_item_icons')
  ...
  puts "Found #{file_names.length} recipe item icon files"
  puts
