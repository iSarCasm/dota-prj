...
CATEGORY_URL = "#{BASE_URL}/Category:Dota_2_hero_minimap_icons"
OUTPUT_DIR = 'minimap_icons'
...
def extract_file_links(html_content)
  ...
  # Pattern: /commons/File:Name_mapicon_dota2_gameasset.png
  html_content.scan(%r{/commons/File:([^"'\s<>]+\.png)}i) do |match|
    file_name = match[0]
    # Filter for mapicon files
    if file_name.include?('mapicon') && file_name.include?('dota2')
  ...
  html_content.scan(/href=["']([^"']*File:[^"']*\.png[^"']*)["']/i) do |match|
    href = match[0]
    if href.include?('File:') && href.include?('mapicon') && href.include?('dota2')
...
def construct_image_url(file_name)
  ...
  # We'll try the direct path first, then use API if needed
...
def main
  puts "Starting Dota 2 hero minimap icon downloader..."
  ...
  puts "Found #{file_names.length} minimap icon files"
  puts
